...
1. 用于路由解析，**对外暴露 REST API 接口**；负责**接收并转发**请求、参数校验；
    ...
    5. `BaseController/CommonResult` 类：用于**统一封装返回对象**，供所有的 controller 继承，主要提供了返回成功或失败对象的几种方法。
...
本质上相当于传统的 Servlet 和 Struts 中的 action，主要体现在 Controller 层，用来拦截和转发用户请求、返回响应结果。[Spring MVC 常用基本注解](#声明 Bean 的注解)
...
1. `'singleton`：单例模式，默认值，在整个**Spring IoC 容器中**只有一个**共享**的 Bean 实例，由 `BeanFactory` 维护。一旦创建成功，**可重复使用**。存储在高速缓存中，用于**无会话状态**的 Bean（如 DAO 层、Service 层）。
...
`@Aspect`：用于定义切面；切面是通知和切点的结合，分别定义了**何时、何处**应用通知功能。
...
2. `SUPPORTS`：支持**当前**事务，如果当前没有事务，以非事务方式执行；事务将不会发生回滚；
3. `'MANDATORY`：支持**当前**事务，如果当前没有事务，抛出异常；
4. `REQUIRES_NEW`：新建事务，如果当前存在事务，把当前事务**挂起**；
5. `NOT_SUPPORTED`：以非事务方式执行操作，如果当前存在事务，把当前事务**挂起**；事务将不会发生回滚；
...
3. **代理模式**：AOP 的实现用到 **JDK 动态代理**和 CGLIB 字节码生成技术；
...
Swagger（音S歪戈）：可动态地根据**注解**生成在线API文档。是一套基于 OpenAPI 规范构建的开源工具，可帮助设计、构建、记录及使用 Rest API。
...
4. 修改 MyBatis Generator 注释的生成规则，运行代码生成器重新生成 mbg 包中的代码
    ...
    3. 使用`field.addJavaDocLine()`，给实体类的字段添加注释、Swagge 注解 `@ApiModelProperty` 来取代原来的方法注释。
...
3. SSO 单点登录；
4. Apache **Shiro** 权限管理框架：强大易用的 Java 安全框架，提供了认证、授权、加密和会话管理功能。
...
- 最终目的是：为第三方应用颁发一个**有时效性**的令牌 Token，使得第三方应用能够（通过该令牌）获取相关的资源，如该用户在某一网站上存储的私密的资源（如照片）。
...
> 整合 Spring Security 和 JWT 实现后台用户的**登录和授权功能**，同时改造 `Swagger-UI` 的配置使其可自动发送**登录令牌**。
...
2. `configure(HttpSecurity httpSecurity)`：用于读取并配置需拦截的 URL 路径、**白名单 URL 资源路径**，是否允许跨域的OPTIONS请求，配置自定义权限 JWT 过滤器、自定义权限**拒绝处理器**、抛出异常后的处理器、动态权限**校验过滤器**等；
...
3. 添加 `UmsAdminController` 类：实现后台用户登录、注册及获取权限的接口；为接口中的方法添加**访问权限**。
