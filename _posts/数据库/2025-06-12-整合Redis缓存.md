...
在系统访问量越来越大之后，往往最先出现瓶颈的往往是数据库。而为了减少数据库的压力，**可以选择让产品砍掉消耗数据库性能的需求**。当然，也可以选择如下方式优化：

> 在这里，暂时不考虑优化数据库的硬件，索引等等手段。
...
在我们访问数据库时，我们会创建 `dao` 包，存放每个 DO 对应的 Dao 对应。那么对于每一个 CacheObject 类，我们也会创建一个其对应的 Dao 类。例如说，UserCacheObject 对应 UserCacheObjectDao 类。
...
② 项目中还保留了部分地方使用本地缓存，例如说：**短信客户端、文件客户端、敏感词**等。主要原因是，它们是**“有状态”的 Java 对象**，无法缓存到 Redis 中。
...
1. 项目启动时，**初始化缓存**：从数据库中读取数据，写入到本地缓存（例如说一个 Map 对象）
2. 数据变化时，实时**刷新缓存**：（例如说通过管理后台修改数据）重新从数据库中读取数据，重新写入到本地缓存

实战案例：以 [角色模块 (opens new window)](https://github.com/YunaiV/yudao-cloud/blob/master/yudao-module-system/yudao-module-system-biz/src/main/java/cn/iocoder/yudao/module/system/service/permission/RoleServiceImpl.java)为例，讲解如何实现角色信息的本地缓存。

![实现原理](../assets/01-cloud.png)
...
② 在 [RoleServiceImpl (opens new window)](https://github.com/YunaiV/yudao-cloud/blob/master/yudao-module-system/yudao-module-system-biz/src/main/java/cn/iocoder/yudao/module/system/service/permission/RoleServiceImpl.java)类中，实现 `#initLocalCache()` 方法，通过 `@PostConstruct` 注解，在**项目启动时**进行本地缓存的初始化。
...
为什么需要使用 [Spring Cloud Bus (opens new window)](https://spring.io/projects/spring-cloud-bus)来实时刷新缓存？

- 考虑到**高可用**，线上会部署多个 JVM 实例，需要通过 RocketMQ **广播**到所有实例，实现本地缓存的刷新。

![实时刷新缓存](../assets/02-cloud.png)
...
① 新建 [RoleProducer (opens new window)](https://github.com/YunaiV/yudao-cloud/blob/master/yudao-module-system/yudao-module-system-biz/src/main/java/cn/iocoder/yudao/module/system/mq/producer/permission/RoleProducer.java)类，RoleRefreshMessage 的 Producer 生产者。
...
② 在数据的新增 / 修改 / 删除等写入操作时，需要使用 RoleProducer 发送消息。

![localsession_03](../assets/localsession_03.png)

#### RoleRefreshConsumer

新建 [RoleRefreshConsumer (opens new window)](https://github.com/YunaiV/yudao-cloud/blob/master/yudao-module-system/yudao-module-system-biz/src/main/java/cn/iocoder/yudao/module/system/mq/consumer/permission/RoleRefreshConsumer.java)类，RoleRefreshMessage 的 Consumer 消费者，刷新本地缓存。
