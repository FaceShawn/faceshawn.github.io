...
for (int i = 1; i <= n; i++) { // i = 1
	// 表示[0, i-1]位置的累加和
	preSums[i] = preSums[i-1] + nums[i-1]; //i-1
}
```
...
- 见 [nSum](#数组)
- 见 [判断全串是否回文](#回文串)
- [344.原地反转字符数组 O(1) 额外空间](https://leetcode.cn/problems/reverse-string/)：数组，双指针swap()

<img src="assets/towards_double_pointer.gif" alt="344.反转字符串" style="zoom: 44%;" />

- [剑指 Offer 21.调整数组顺序使奇数位于前半部分](https://leetcode.cn/problems/diao-zheng-shu-zu-shun-xu-shi-qi-shu-wei-yu-ou-shu-qian-mian-lcof/)：头指针指向第一个偶数，尾指针指向倒数第一格奇数；`while(i < j)`。
...
- [349.两个数组的交集](https://leetcode.cn/problems/intersection-of-two-arrays/)：`insert` set 容器 `record` 存储 `a1` 的元素，遍历 `a2` `find` 查找是否有相同的元素，如果有，set 容器 `resultSet` 进行存储，用 `iterator` 将 resultSet 转换为 vector 类型。【】
