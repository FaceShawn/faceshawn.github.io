...
1. 布尔型 `boolean` 占用大小根据实现 JVM 不同有所差异，官方手册没有明确说明，**逻辑上占1Byte**：
    - 根据《Java虚拟机规范》里的结论，如果boolean单独使用、最终被编译为int 类型，则占4字节；如果以boolean数组使用、最终会被编码为 byte 数组，则占1字节；
...
1. 根本：数据类型；引用数据类型（面向对象的**类**），有类的特性（封装、继承、多态等）。
...
2. 当要访问包装类对象的真实数据值时，如进行数学运算、比较大小、输出对象的值、三目运算符数据类型对齐。

频繁拆/装箱，也会严重影响系统的性能，应尽量避免不必要的拆/装箱操作。
...
```Java
// 装箱
Integer ig1 = new Integer(128); // 手动装箱，基本数据类型 => 堆内存中的包装类对象
Integer ig2 = 128; // 自动装箱，调用 Integer.valueOf(128)；Double、Float的valueOf()类似

// 比较包装类对象
ig1 == ig2? // false，比较引用，常量池外的值不复用，两个不同对象的地址不同；若为[-128, 127]，则复用已有对象，地址相同，返回true
ig1.equals(ig2)? // true，不同对象的值相等

// 拆箱
int i3 = ig2.intValue(); // 手动拆箱，包装类对象 => 基本数据类型
int i3 = ig2; // 自动拆箱，调用 ig2.intValue()/xxxValue()
...
Integer ig1 = Integer.valueOf(str); // 字符串 => 包装类对象
String str = ig1.toString(); // 包装类对象 => 字符串
...
定义：指把对象的成员变量/属性隐藏在内部，不允许外部对象直接访问，通过（可被外界访问的）public 方法来操作属性。控制对外隐藏和暴露哪些数据。基本单位是类。如 POJO 类的 getter/setter 方法。
...
作用：为了避免精度丢失，可用 `BigDecimal` 来进行**浮点数间的运算和比较大小**；
