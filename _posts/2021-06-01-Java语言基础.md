...
1. 使 JVM 可实现**字符串常量池**：复制 String 变量、new 同一字面量、不同的字符串变量指向相同的字面量，都是指向字符串常量池中（同一内存地址的）同一对象。字符串常量池的好处：
...
3. 内存地址不变，因为 **hashcode** 由内存地址间接得到，因此保证了 `hashcode` 的唯一性：创建 String 时 `hashcode` 就被缓存了，不必每次重新计算，处理速度快，很适合作为 Map 的键。
...
3. 占内存大小：字符常量只占 2 个字节；字符串常量取决于长度。
