...
日志框架： JCL、SLF4J、Jboss-logging、jUL、~~log4j、~~log4j2、**logback** 等。
...
![springboot集成LogStash的步骤](assets/ELK.jpg)
...
1. **系统可用性降低：** 系统可用性在某种程度上降低；在加入 MQ 前，不用考虑消息丢失或说 MQ 挂掉等情况，但引入 MQ 后就需要去考虑了！
2. **系统复杂性提高：** 加入 MQ 后，需保证消息没有被重复消费、处理消息丢失的情况、保证消息传递的顺序性等问题！
3. **一致性问题：**消息队列带来的异步确实可提高系统响应速度。但万一消息的真正消费者并没有正确消费消息会导致数据不一致的情况!

分类：

1. RocketMQ：阿里巴巴开源的一款高性能、高吞吐量的分布式消息中间件；
2. **Kafaka**：一种分布式的，基于发布 / 订阅的消息系统；熟悉Redis、Kafka等大数据组件
3. RabbitMQ：由 erlang 开发的基于 AMQP（Advanced Message Queue 高级消息队列）协议实现的消息队列。

### 读写分离

主要是为了将数据库的读和写操作分不到不同的数据库节点上。主服务器负责写，从服务器负责读。另外一主一从或一主多从都可以。可大幅提高读性能，小幅提高写的性能。因此更适合单机并发读请求比较多的场景。

### 分库分表

作用：为了解决由于库、表数据量过大，而导致数据库性能持续下降的问题。

常见的分库分表工具：

- `MyCAT`（基于 Cobar）
- `sharding-jdbc`（当当）：推荐用，是一款轻量级 `Java` 框架，以 `jar` 包形式提供服务，不需做额外运维，且兼容性很好。
- Sharding-Sphere
- `TSharding`（蘑菇街）
- `Cobar`（阿里巴巴）
...
负载均衡系统：通常用于将任务（如用户请求处理）分配到多个服务器处理以提高网站、应用或者数据库的性能和可靠性。
...
高可用：描述的是一个系统在大部分时间都是可用的，可以为我们提供服务的。高可用代表系统即使在发生硬件故障或者系统升级的时候，服务仍然是可用的 。
