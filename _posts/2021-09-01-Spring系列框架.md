...
1. `DO`（Data/Domain Object）：数据/领域对象，由 DAO 层向上（Service 层）传输的，从现实世界中抽象出的有形或无形的业务实体。
2. ~~`PO`~~（Persistent Object）：持久化对象，与持久层（通常是关系型数据库表）数据结构一 一对应的 POJO 类，由 DAO 层向上（Service 层）传输的，数据源对象。
3. `BO`（Business Object）：业务对象，由 Service 层向上（Controller 层）传输的，封装业务逻辑的对象。通常由 DO 转化、整合而来，可包含多个 DO 的属性，也可只包含一个 DO 的部分属性。
...
用于处理数据业务，存放自定义的 `Mapper` 映射器接口，对应 `resources/dao`下的映射文件（`XxxDao.xml`）。MyBatis 可~~直接在配置文件中~~动态实现接口的方法，所以不需要 `daoImpl`；

直接进行数据库的读写操作，返回与数据库表一一对应的数据对象 DO。
...
- Repository 是相对对象而言，蕴含着真正的OO概念，即一个数据仓库角色，负责所有对象的持久化管理；

- DAO 则是相对数据库而言，DAO直接负责数据库的存取工作。

##### DTO 数据传输对象层

用于 Service 层与 Web 层间的传输。存放自定义的传输对象，如请求参数类型定义和返回结果。
...
MyBatis 的代码生成器，可根据数据库生成 `model` 实体类、常用的单表查询 `XxxMapper` 接口及对应的 `XxxMapper.xml`、和 `Example`；自定义的 mapper 接口放在 `dao/XxxDao.java` 中；

1. `resources/generatorConfig.xml` 见 resources [配置文件](#其它模块)；设置`generator.properties` 中配置 JDBC 数据库连接；
...
1. 用于路由解析，**对外暴露 Rest API 接口**；负责**接收并转发**（原始数据和）请求；
    ...
    3. `@Valid` 等用于搭配 [参数校验](#整合HibernateValidator)（具体通过在 POJOs 对象的字段上加注解实现）；
...
- `@RequestMapping(value = "user", method = RequestMethod.GET)`：可标注在 Controller 控制器**类上**和方法上，用于映射 HTTP 请求。
...
- `@PathVariable`：用于从 URI（Uniform Resource Identifier，统一资源标识符，如身份证号）中**获取路径参数**。最适合 RESTful Web 服务。可在一个方法中定义多个。

区别：

1. 都用于提取方法参数；
2. 获取参数值的方式不同，`@RequestParam` 从请求携带的参数中获取参数，而 `@PathVariable` 从请求的 URI 中获取。

参数最终会通过 `HandlerMethodArgumentResolver` 来解析：

<img src="assets/v2-0190130cdbcc6d78aeb926597e58bcaf_720w.jpg" alt="img" style="zoom:80%;" />
...
3. `@RequestBody`： 用于将传入的 HTTP 请求体与注解的方法参数（中的对象）绑定。一个请求方法只可有一个 `@RequestBody`，但可有多个 `@RequestParam` 和 `@PathVariable`。响应体另见[@ResponseBody](#@Controller)。
...
[Spring Boot 整合 MyBatis](#Spring Boot 整合 MyBatis)
...
1. `@Controller`：常用于 Controller 控制器类上，作用见 [Controler 控制层](#Controler 控制层)。单独使用时（不加 `@ResponseBody`）返回一个视图/页面，用于（传统 Spring MVC）前后端不分离的情况。
   - 若扫描该类下有 `@RequestMapping` 方法，根据注解信息生成对应的处理器对象。
2. `@ResponseBody`：常用于 Controller 方法上，返回具体数据类型而非跳转；将返回的对象转换为指定格式，并写入（填充到） HTTP （响应对象的）响应 body 中，常用于构建 RESTful 的 api，返回 `JSON` 或 XML 数据。请求体另见[@RequestBody](#@RequestXxx 系列)。
3. `@RestController` = `@Controller` + `@ResponseBody`：表示 [REST 风格](#REST 常用注解)的控制器 Bean。Springboot 会把这个类当成 controller 进行处理，然后把所有返回的参数放到 ResponseBody 中。返回 JSON 或 XML 格式（由客户端的 `ACCEPT` 请求头决定）的对象数据，并直接写入 HTTP 响应体中。属于 `RESTful Web/API` 服务，最常用的前后端分离框架的情况。

##### REST 常用注解

处理请求的注解：见 [REST 常用注解](#REST 常用注解)
...
2. **声明式事务**：分离事务管理和业务逻辑代码，本质是通过 **AOP**，对方法前后进行拦截，将事务处理（开启、提交或回滚操作）的功能编织到拦截的方法中（即在目标方法开始前加入一个事务，在执行完目标方法后根据执行情况提交或回滚事务）。不需编程，只需通过在 XML 文件中配置、或直接基于注解实现。实现声明式事务的方式：
   ...
   2. 基于 `@Transactional` 的全注解方式的：简化，使用最多。
      ...
      2. 只能作用于 `public` 方法：方法的事务会覆盖类的事务配置信息。
...
2. `ISOLATION_READ_UNCOMMITTED`：读未提交，允许另外一个事务可看到这个事务未提交的数据；
3. `ISOLATION_READ_COMMITTED`：读已提交，保证一个事务修改的数据提交后才能被另一事务读取，而且能看到该事务对已有记录的更新；
4. `ISOLATION_REPEATABLE_READ`：可重复读，保证一个事务修改的数据提交后才能被另一事务读取，但不能看到该事务对已有记录的更新；
...
1. 部署方便：内置多种 Web 容器，如 [Tomcat](#Tomcat) （作为默认的嵌入式 HTTP 服务器/ `Servlet` 容器）、 Jetty、Undertow，可轻松开发和测试 Web 应用程序；直接运行入口函数即可启动项目/作为独立应用程序运行；
...
`Lombok` 插件：**简化** `POJOs` 类**封装**，通过为实体类添加注解、来自动生成（并代替）通用方法；减少冗余代码。缺点：提高阅读源码的门槛，继承父类时 `equals()` 可能会出错。
...
1. `@Data`：用在类上，相当于同时使用 `@Getter` / `@Setter`、`@ToString`、`@EqualsAndHashCode`、`@RequiredArgsConstructor` 等多个注解，生成通常与简单`POJO`关联的所有样板和 `bean`；
2. `@EqualsAndHashCode(callSuper=false)`：用在类上，自动生成 `equals()` 方法和 `hashCode()` 方法，包括所有非静态变量和非 transient 的变量。
    1. 默认仅使用子类中定义的属性、不调用**父类**的方法，可能会导致比较对象出错；如，~~父类中有 id 等共同属性~~，实际上并不相等的对象在比较时，却因为 lombok 自动生成的 `equals()` 和 `hashCode()`方法而判定为相等，从而导致出错；
        1. 可通过设置 `callSuper=true` 解决这一问题，让其生成的方法中调用父类方法；
        2. 或可用 `@Getter @Setter @ToString` 代替 `@Data`、并自定义 `equals(Object other)` 和 `hashCode()`方法；
...
4. `@Getter` / `@Setter`：用在类/属性上；
     ...
     2. 用在类上时， `@Setter(AccessLevel.PUBLIC) `，指定访问级别为`PUBLIC，PROTECTED，PACKAGE，PRIVATE，NONE`。
...
3. Controller 层：添加 `UmsMemberController`，根据电话获取验证码的接口和校验验证码的接口；控制层方法中 `@Redis` 标注的参数（如，`@Redis(key = "redisKey") String redisValue` ），值应从 Redis 中获取，不用从请求参数中获取。
    ...
    <img src=”assets/v2-a226f96dff62a33e3b705e07b51dc319_720w.jpg“ alt=”img“ style=”zoom:42%;“ />
   ![img](assets/v2-a226f96dff62a33e3b705e07b51dc319_720w.jpg)
...
`javax.validation` 支持的注解，被注释的元素需满足：

1. `@NotNull(message = "name不能为空")`：元素不能为 null；可为 empty（没有 size 的约束）；
   1. `@Null`：元素必须为 null；
   2. ~~`@NotEmpty`~~：用于字符串、集合、Map、数组，不能为 null 或 空`empty（size>0）`（String、Collection、Map的 `isEmpty() `方法）；
...
4. `@Size(min=4, max=15)`：元素大小必须在指定范围内；可是字符串、数组、集合、map等；
   ...
   2. `@DecimalMin(value)`/`@DecimalMax(value)`：必须是一个（BigDecimal 的字符串表示形式的）的数字，可以是小数，值必须 `>= / <=` 指定值；
...
3. 支持**分组**验证：在方法参数验证时，根据不同的分组采用不同的验证机制。
...
4. `@ApiParam`：修饰接口参数；
5. `@ApiModelProperty`：修饰实体类的属性，用于实体类是请求参数或返回结果时；
...
ELK 是常用的日志处理系统。SpringBoot 配置 LogBack，将日志发送给 Logstash，再转发给 Elasticsearch。[SpringBoot 应用整合 ELK 实现日志收集](https://github.com/macrozheng/mall-learning/blob/master/docs/technology/mall_tiny_elk.md)

1. `Elasticsearch`（ES 搜索引擎）：用于存储收集到的日志信息；
2. `Logstash` 日志收集器：用于收集日志；可理解为一个管道，或中间件。通过配置 `logback-spring.xml` 的 `<property> <appender> <logger>` 标签，从定义的输入源 inputs（stdin、日志文件、数据库等）读取信息，经过 filters 过滤器处理，输入到定义好的outputs 输出源（stdout、elesticsearch、HDFS等）。[springboot 集成 LogStash 的步骤](https://www.yisu.com/zixun/89177.html)
...
5. `<appender name="" class="">`：日志打印组件。让应用知道怎么打、打印到哪里、打印成什么样。通过`logger`或`root`的`appender-ref`指定某个具体的`appender`；
...
6. `BeanUtil` JavaBean 的工具类：用于 Map 与 JavaBean 对象的互相转换及对象属性的拷贝；
...
1. 通过 `SqlSessionFactoryBuilder` 从 `mybatis-config.xml` 配置文件中构建出 `SqlSessionFactory`；
2. `SqlSessionFactory` 开启一个 `SqlSession`，通过 `SqlSession` 实例获得 `Mapper` 对象并且运行 `Mapper` 映射的 SQL 语句；
...
1. `@Mapper`：用于定义 Mybatis 映射器接口（Dao 层接口）。告诉 Spring 框架此接口的实现类由 Mybatis 负责创建，并将其实现类对象存储到 Spring 容器中。不支持重载。
2. `@MapperScan(basePackages = "top.snailclimb.dao")` ：（当 mapper 接口较多时），用于 Spring Boot 主启动类或 **Java 配置类**上，表示动态扫描指定包（DAO 接口所在包）下的 mapper 接口，相当于每个 mapper 接口上都标注 `@Mapper`。见 [SqlSessionFactoryBuilder 类](#SqlSessionFactoryBuilder 类)。
...
2. `XML` 映射文件（`XxxDao.xml/XxxMapper.xml`）：用 XML 来映射复杂语句（如：嵌套联合语句？映射）。与映射器接口同名，MyBatis 会自动查找并加载。
...
1. `@Select`：用于方法上，取代 XML 中的 `<select>` 标签。
    1. `@Param`：用于方法参数向映射器（动态 SQL `${}、#{}`）传参；方法有两个或以上的参数时一定要加，不然 Mybatis 识别不了；
...
**id 是否可重复**：如果配置了namespace，则 id 可重复； 如果没有配置，则不能重复，namespace 不是必须的。原因就是 namespace+id 是作为 Map 的 key。

1. `<cache>`：命名空间的缓存配置。MyBatis 的缓存分为一级缓存和二级缓存，一级缓存放在session 里，默认就有，二级缓存放在它的命名空间里，默认是打开的，使用二级缓存属性类需实现 Serializable 序列化接口（可用来保存对象的状态），通过 `<cache>` 配置。

2. `<cache-ref>`：引用其他命名空间的缓存配置。

3. `<resultMap> `：描述如何从数据库**结果集**中加载对象。最复杂也最强大的元素。逐一定义列名和对象属性名间的映射关系，MyBatis 通过**反射**创建对象，同时用反射给对象的属性逐一赋值并返回。包括的子标签有：

    1. `<constructor>`：用于在实例化类时，注入结果到构造方法中；
        - Executor 执行器、批处理；
        - 类型处理器（typeHandlers）：处理枚举类型；
    2. `<id />`：一个 id result；标记出作为 ID 主键的结果，可帮助提高整体性能；
    3. `<result />`：注入到字段或 JavaBean 属性的普通结果；用于设置数据库中表的 `column` 列名、`jdbcType`、`property` POJO 实体类（Bean）中的属性名；
...
`#{}`：是 SQL 的**参数占位符**，预编译处理时会被替换为 `?`，在 SQL 执行前，调用~~`JDBC`中的~~ `PreparedStatement` 的 set 方法，按序、安全地设置参数值。
...
- `keyProperty`：用于主键回填，将自增长的主键设置为 `LAST_INSERT_ID()` 的返回值；打印新插入记录的 id。唯一标记一个属性，MyBatis 会通过 `getGeneratedKeys()` 的返回值或通过 insert 语句的 `<selectKey>` 子元素设置它的键值，默认：unset。
- ~~`keyColumn`~~：通过生成的键值设置表中的列名，仅在某些数据库（如 PostgreSQL）是必须的，当主键不是表的第一列时需设置。

`<insert>` 语句的 `<selectKey>`：

在 MySQL 中用函数来自动生成插入表的主键，而且需方法返回这个生成主键，如用 MyBatis 的 selectKey标签。是为了解决 Insert 数据时不支持**主键自动生成**的问题，可很随意的设置生成主键的方式。尽量不用，很麻烦。
...
**SQL语句构建器**：通过 **SQL 工具类**处理动态 SQL 典型问题，如加号、引号、换行、格式化问题、嵌入条件的逗号管理及 AND 连接。
...
2. 用分页插件：基本原理是用 MyBatis 提供的插件接口，实现自定义插件，在插件的拦截方法内拦截并重写待执行的 SQL，根据 dialect 方言，添加对应的物理分页语句和参数。
...
1. Session 和 Cookie；
2. `Spring Security + OAuth2`：强大的可高度定制的**认证和授权框架**，对 Spring 应用来说是一套 Web 安全标准。
