...
> 摘要：1. 基本语法、常量和变量（final、static 关键字）、数据类型、面向对象（封装、继承、多态、接口）；2. 异常、泛型、反射、注解；3. IO和序列化、Socket 网络编程、JDK 新特性；4. 常用类等。
...
- 用于序列化和反序列化的类必须实现 `java.io.Serializable` 接口：用于实现 Java 类的序列化操作而提供的一个**语义级别**的接口。没有任何方法或字段，只是用于标识可序列化的语义。
